...
from handlers.txt import summ_text, complete_transaction_text, error_number_text, error_negative_number_text
...
async def msg_add_summ(message: types.Message, state: FSMContext):
    if is_number(message.text):
        if float(message.text) > 0:
            ...
            await message.answer(error_negative_number_text, input_field_placeholder='Введи сумму')
    else:
